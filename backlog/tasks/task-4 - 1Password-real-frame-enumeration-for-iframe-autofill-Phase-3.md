...
Trial 2026-09-21 00:56-01:07 (signed build from 761c4b6 with TASK-75, pid 68860, 1Password 8.12.26.40, ExtensionConsoleLogPublic set, fixture http://127.0.0.1:8471/ + cross-origin frame http://localhost:8472/, normal window). Per frame kind (user observation): top-level form — control shown, menu opens ('No items to show'); cross-origin http iframe — control shown in the input, clicking it does NOTHING (no menu); srcdoc iframe — no control; script-filled about:blank iframe — no control. Log: the fixture load produced NO '[Tabs] Could not collect all frames that were initially found' in the Personal worker. The line appeared once, 01:00:54.085, in the PRIVATE worker (84f2a174...) on whatever page was open in the Private window (site not recorded; passkey/Webauthn lines around it), so it is not caused by srcdoc/about:blank frames on this fixture. Reading: the empty-URL frames are real and cost 1Password its control there (matches the Sep 13 measurement: no content script in those frames), but they are not shown to be the source of the 'Could not collect all frames' line; and a second, separate defect exists — in a cross-origin http iframe the content script runs but the inline menu never opens. AC #4 still lacks the named real iframe-login site.

2026-09-21 close-out. The Private-window page in the trial above was reddit.com (user). AC #4 deviation, accepted: no real site with its login form inside an iframe was identified, so the real-site leg is reddit.com in a Private window — which is where the only 'Could not collect all frames that were initially found' line of the capture appeared (01:00:54.085); the controlled fixture covers the three frame kinds. AC #5: hypothesis REJECTED as the explanation of the log line — the fixture has a srcdoc and a script-filled about:blank frame with url '' and no content script, and loading it (and using 1Password on it) logged no such line. Next hypothesis, handed to TASK-102: the line comes from reddit.com's own frame set (third-party sign-in / ad / about:blank frames that appear and vanish, or frames 1Password counted at document_start that were gone or unanswering by fan-out time), possibly together with the Private worker's 'a.providers' rejections logged in the same seconds. The user-visible defects the trial did establish are filed separately: TASK-100 (no content script, hence no 1Password control, in srcdoc/about:blank frames) and TASK-101 (control drawn in a cross-origin http iframe but the inline menu never opens).
<!-- SECTION:NOTES:END -->

## Final Summary

<!-- SECTION:FINAL_SUMMARY:BEGIN -->
Investigation task, no product fix. Established: WebKit provides webNavigation.getAllFrames/getFrame natively (polyfill fallbacks removed); srcdoc and about:blank frames are enumerated with url '' and get no content script (measured by testFrameKindsAsReportedByNativeGetAllFrames). Signed-build trials with real 1Password: 2026-09-13 exposed the ported-origin throw (fixed by TASK-75); 2026-09-21 showed per frame kind — top-level: menu works; cross-origin iframe: control but no menu (TASK-101); srcdoc/about:blank: no control (TASK-100). The empty-URL hypothesis for 'Could not collect all frames' is rejected: the fixture never logged it, reddit.com in Private did (TASK-102). Results in docs/1password-integration-plan.md Phase 3.
